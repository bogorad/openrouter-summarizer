...
export function createFloatingIcon(clickX, clickY, onIconClick, onIconDismiss) {
  ...
  if (typeof onIconClick !== 'function' || typeof onIconDismiss !== 'function') {
    console.error('[LLM FloatingIcon] createFloatingIcon failed: Required callbacks missing.');
    return;
  }
  ...
  try {
    ...
    } else { throw new Error("chrome.runtime API not available"); }
  } catch (e) {
  ...
  iconImg.onerror = function() {
    ...
    if (!floatingIcon || floatingIcon.querySelector(`.${FALLBACK_ICON_CLASS}`)) return; // Prevent adding multiple fallbacks
    const fallback = document.createElement('span');
    fallback.className = FALLBACK_ICON_CLASS;
    fallback.textContent = '💡';
    fallback.style.pointerEvents = 'none';
    floatingIcon.appendChild(fallback);
    if (DEBUG) console.warn('[LLM FloatingIcon] Failed to load icon image, using fallback.');
  };
