...
function initializeChat() {
    ...
    chrome.runtime.sendMessage({ action: "getChatContext" }, (response) => {
        ...
        if (response && Array.isArray(response.models) && response.models.length > 0) {
            models = response.models; populateModelDropdown(response.summaryModel);
            if (response.domSnippet && response.summary) {
                chatContext.domSnippet = response.domSnippet; chatContext.summary = response.summary; chatContext.summaryModel = response.summaryModel || selectedModelId;
                if (DEBUG) console.log('[LLM Chat] Context received successfully. Populating UI.');
                messages = [{ role: "assistant", content: chatContext.summary, model: chatContext.summaryModel }]; renderMessages();
            } else { console.warn('[LLM Chat] Context received from background is incomplete.'); messages = []; renderMessages(); showError('Could not load initial summary context.', false); }
            focusInput();
        } else { console.error('[LLM Chat] Failed to get valid context or models list.', response); showError('Error: Could not load models list from settings.'); disableChatFunctionality(); }
    });
...
function populateModelDropdown(preferredModelId) { if (!modelSelect) return; modelSelect.innerHTML = ''; modelSelect.disabled = false; if (!models || models.length === 0) { console.error("[LLM Chat] No models available."); const opt = document.createElement('option'); opt.value = ""; opt.textContent = "No models configured"; opt.disabled = true; modelSelect.appendChild(opt); modelSelect.disabled = true; selectedModelId = ""; return; } models.forEach(modelId => { const opt = document.createElement('option'); opt.value = modelId; opt.textContent = modelId; modelSelect.appendChild(opt); }); if (preferredModelId && typeof preferredModelId === 'string' && models.includes(preferredModelId)) { modelSelect.value = preferredModelId; selectedModelId = preferredModelId; } else if (models.length > 0) { modelSelect.value = models[0]; selectedModelId = models[0]; } else { selectedModelId = ""; } if (DEBUG) console.log(`[LLM Chat] Model dropdown populated. Initial selection: ${selectedModelId}`); modelSelect.onchange = function() { selectedModelId = this.value; if (DEBUG) console.log(`[LLM Chat] Model selection changed to: ${selectedModelId}`); }; }
    ...
    if (messages.length === 0) { wrap.innerHTML = '<div class="msg system-info">Chat started. Ask a follow-up question...</div>'; }
        ...
        messages.forEach((msg, index) => {
            ...
            if (msg.role === 'assistant') {
                ...
                let htmlContent = ''; // Initialize
                let contentToProcess = (typeof msg.content === 'string') ? msg.content : '';

                // --- NEW: Strip potential code fences before parsing ---
                const codeFenceRegex = /^\s*```(?:markdown)?\s*\n([\s\S]*?)\n\s*```\s*$/;
                const match = contentToProcess.match(codeFenceRegex);
                if (match && match[1]) {
                    // If matched, use the content inside the fences for parsing
                    contentToProcess = match[1].trim();
                    if (DEBUG) console.log('[LLM Chat Render] Extracted content from Markdown code fence before parsing.');
                }
                // --- END: Strip fences ---
                ...
                // --- Markdown Handling ---
                if (typeof marked !== 'undefined') {
                    // ALWAYS try to parse assistant content as Markdown
                    if (DEBUG) console.log('[LLM Chat Render] Parsing assistant message content with marked.');
                    try {
                         // Parse the (potentially stripped) content
                         htmlContent = marked.parse(contentToProcess);
                    } catch (e) {
                         console.error("[LLM Chat Render] Error parsing Markdown:", e);
                         htmlContent = contentToProcess.replace(/\n/g, '<br>'); // Fallback on error
                    }
                } else {
                ...
                contentSpan.innerHTML = htmlContent; // Set the parsed HTML
...
// ==== CHAT FORM HANDLING ====
function handleFormSubmit(e) { if (e) e.preventDefault(); if (DEBUG) console.log('[LLM Chat] Form submit triggered.'); if (streaming) { if (DEBUG) console.log('[LLM Chat] Ignoring submit: Already streaming.'); return; } if (!selectedModelId) { showError("Cannot send message: No model selected or configured.", false); return; } if (!chatInput) { console.error("Cannot send: Input field not found!"); return; } const text = chatInput.value.trim(); if (!text) return; messages.push({ role: "user", content: text }); renderMessages(); chatInput.value = ''; chatInput.style.height = '40px'; chatInput.focus(); streaming = true; currentStreamModel = selectedModelId; currentStreamRawContent = ""; const messagesWrap = chatMessagesInnerDiv; if (!messagesWrap) { console.error("Cannot append streaming elements: .chat-messages-inner not found."); streaming = false; return; } const modelLabelDiv = document.createElement('div'); modelLabelDiv.className = 'assistant-model-label'; modelLabelDiv.textContent = `Model: ${currentStreamModel}`; messagesWrap.appendChild(modelLabelDiv); let streamContainer = document.createElement("div"); streamContainer.className = 'msg assistant'; streamContainer.innerHTML = `<span class="assistant-inner" id="activeStreamSpan"></span>`; messagesWrap.appendChild(streamContainer); currentStreamMsgSpan = streamContainer.querySelector('#activeStreamSpan'); if (!currentStreamMsgSpan) { console.error("Failed to create #activeStreamSpan!"); streaming = false; return; } showLoadingIndicator(true); scrollToBottom(); let apiMessages = []; const isFirstUserTurn = messages.filter(m => m.role === 'user').length === 1; if (isFirstUserTurn && chatContext.domSnippet && chatContext.summary) { apiMessages.push({ role: "system", content: "You are a helpful assistant... Please format responses using Markdown." }); apiMessages.push({ role: "user", content: `Original HTML Snippet:\n\`\`\`html\n${chatContext.domSnippet}\n\`\`\`\n\nInitial Summary (from model ${chatContext.summaryModel || 'unknown'}):\n${chatContext.summary}` }); apiMessages.push({ role: "user", content: text }); } else { const historyLimit = 10; apiMessages = messages.slice(-historyLimit).map(m => ({ role: m.role, content: m.content })).filter(m => m.role === 'user' || m.role === 'assistant'); } if (DEBUG) console.log('[LLM Chat] Sending message request to background. Model:', currentStreamModel, 'Payload:', apiMessages); chrome.runtime.sendMessage({ action: "llmChatStream", messages: apiMessages, model: currentStreamModel }); }

// ==== STREAMING HANDLERS ====
function setupStreamListeners() { if (messageListenerAttached) { if (DEBUG) console.log("[LLM Chat] Stream listeners already attached."); return; } if (DEBUG) console.log("[LLM Chat] Setting up stream listeners."); chrome.runtime.onMessage.addListener((req, sender, sendResponse) => { if (!req || !req.action) return; switch (req.action) { case "llmChatStreamChunk": if (streaming && currentStreamMsgSpan && typeof req.delta === 'string') { currentStreamRawContent += req.delta; currentStreamMsgSpan.textContent = currentStreamRawContent; scrollToBottom(); } else if (DEBUG) { console.warn('[LLM Chat] Received chunk conditions not met:', { streaming, currentStreamMsgSpanExists: !!currentStreamMsgSpan, delta: req.delta }); } break; case "llmChatStreamDone": if (streaming) { if (DEBUG) console.log('[LLM Chat] Stream finished. Model reported:', req.model); messages.push({ role: "assistant", content: currentStreamRawContent, model: req.model || currentStreamModel }); streaming = false; currentStreamMsgSpan = null; currentStreamRawContent = ""; currentStreamModel = ""; showLoadingIndicator(false); renderMessages(); focusInput(); } else if (DEBUG) { console.warn('[LLM Chat] Received stream DONE but not streaming.'); } break; case "llmChatStreamError": console.error('[LLM Chat] Stream Error from background:', req.error); showError(`LLM Error: ${req.error || "Unknown failure"}`, false); if (streaming) { renderMessages(); } streaming = false; currentStreamMsgSpan = null; currentStreamRawContent = ""; currentStreamModel = ""; showLoadingIndicator(false); focusInput(); break; } }); messageListenerAttached = true; }
...
function showError(message, isFatal = true) { if (!errorDisplay) { errorDisplay = document.createElement('div'); errorDisplay.id = 'errorDisplay'; errorDisplay.style.display = 'none'; const chatContainer = document.getElementById('chatContainer'); if (chatContainer && chatMessages) { chatContainer.insertBefore(errorDisplay, chatMessages); } else if (chatContainer) { chatContainer.insertBefore(errorDisplay, chatContainer.firstChild); } else { document.body.insertBefore(errorDisplay, document.body.firstChild); } } errorDisplay.style.cssText = 'display: block; color: red; background-color: #ffebee; padding: 10px; border: 1px solid red; border-radius: 4px; margin: 10px auto; width: 80vw; max-width: 800px;'; errorDisplay.textContent = message; if (isFatal) { disableChatFunctionality(); } }
...
function formatChatAsMarkdown() { if (!messages || messages.length === 0) return ""; let markdownContent = `# Chat Export\n\n`; messages.forEach(msg => { if (msg.role === 'user') { markdownContent += `**User:**\n${msg.content}\n\n`; } else if (msg.role === 'assistant') { const modelInfo = msg.model ? ` (Model: ${msg.model})` : ''; markdownContent += `**Assistant${modelInfo}:**\n${msg.content}\n\n`; } }); return markdownContent.trim(); }
