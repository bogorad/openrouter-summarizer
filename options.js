...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function renderModelOptions() {
    if (!modelSelectionArea) return; modelSelectionArea.innerHTML = '';
    if (!currentModels || currentModels.length === 0) { modelSelectionArea.innerHTML = '<p>No models configured. Add one below or save to use defaults.</p>'; if (addModelBtn) addModelBtn.disabled = true; return; }
    else { if (addModelBtn) addModelBtn.disabled = false; }
    const availableModelIds = currentModels.map(m => m.id);
    if (!currentSelectedModel || !availableModelIds.includes(currentSelectedModel)) { currentSelectedModel = currentModels.length > 0 ? currentModels[0].id : ''; }
    currentModels.forEach((model, index) => {
      ...
      group.appendChild(radio); group.appendChild(textInput); group.appendChild(labelInput); group.appendChild(removeBtn); modelSelectionArea.appendChild(group);
    });
  }
  function handleModelRadioChange(event) { if (event.target.checked) { const index = parseInt(event.target.dataset.index, 10); const selectedModel = currentModels[index]; if (selectedModel?.id.trim()) { currentSelectedModel = selectedModel.id.trim(); if (DEBUG) console.log("[Options Debug] Selected model ID changed to:", currentSelectedModel); } else { event.target.checked = false; const firstValidModel = currentModels.find(m => m.id.trim() !== ''); if (firstValidModel) { const firstValidRadio = modelSelectionArea.querySelector(`input[type="radio"][value="${firstValidModel.id}"]`); if (firstValidRadio) { firstValidRadio.checked = true; currentSelectedModel = firstValidModel.id; } else { currentSelectedModel = ''; } } else { currentSelectedModel = ''; } if (DEBUG) console.log("[Options Debug] Selected model ID adjusted after radio change:", currentSelectedModel); } } }
  function handleModelTextChange(event) { const newModelId = event.target.value.trim(); const idx = parseInt(event.target.dataset.index, 10); if (idx >= 0 && idx < currentModels.length) { currentModels[idx].id = newModelId; const labelInput = document.getElementById(`modelLabel_${idx}`); if (labelInput && !labelInput.value.trim()) { labelInput.value = newModelId; currentModels[idx].label = newModelId; } } else { console.error("handleModelTextChange invalid index:", idx); return; } const associatedRadio = document.getElementById(`modelRadio_${idx}`); if (associatedRadio) { associatedRadio.value = newModelId; associatedRadio.disabled = !newModelId; if (associatedRadio.checked) { if (!newModelId) { associatedRadio.checked = false; currentSelectedModel = ''; const firstValidModel = currentModels.find(m => m.id.trim() !== ''); if (firstValidModel) { const firstValidRadio = modelSelectionArea.querySelector(`input[type="radio"][value="${firstValidModel.id}"]`); if (firstValidRadio) { firstValidRadio.checked = true; currentSelectedModel = firstValidModel.id; } } } else { currentSelectedModel = newModelId; } if (DEBUG) console.log("[Options Debug] Selected model ID updated via text input to:", currentSelectedModel); } } }
  function handleModelLabelChange(event) { const newModelLabel = event.target.value.trim(); const idx = parseInt(event.target.dataset.index, 10); if (idx >= 0 && idx < currentModels.length) { currentModels[idx].label = newModelLabel; } else { console.error("handleModelLabelChange invalid index:", idx); } }
  function handleModelRemoveClick(event) { removeModel(parseInt(event.target.dataset.index, 10)); }
  function addModel() { currentModels.push({ id: "", label: "" }); renderModelOptions(); const newIndex = currentModels.length - 1; const newInput = document.getElementById(`modelText_${newIndex}`); if (newInput) { newInput.focus(); } }
  function removeModel(indexToRemove) { if (indexToRemove < 0 || indexToRemove >= currentModels.length) return; const removedModelId = currentModels[indexToRemove].id; currentModels.splice(indexToRemove, 1); if (removedModelId === currentSelectedModel) { const firstValidModel = currentModels.find(m => m.id.trim() !== ''); currentSelectedModel = firstValidModel ? firstValidModel.id : ''; } renderModelOptions(); }

  // --- Language Selection & Autocomplete Functions ---
  function findLanguageByName(name) {
    if (!name || typeof name !== 'string' || !ALL_LANGUAGE_NAMES_MAP || Object.keys(ALL_LANGUAGE_NAMES_MAP).length === 0) {
      // --- DEBUG LOG ---
      if (DEBUG && name?.trim()) console.warn(`[Options Debug] findLanguageByName: Map empty or not loaded when searching for "${name}". Map keys:`, Object.keys(ALL_LANGUAGE_NAMES_MAP || {}).length);
      // --- END DEBUG LOG ---
      return undefined;
    }
  ...
  function filterLanguages(query) {
    ...
    if (!lowerQuery || !ALL_LANGUAGE_NAMES_MAP || Object.keys(ALL_LANGUAGE_NAMES_MAP).length === 0) return [];
    // --- DEBUG LOG ---
    if (DEBUG) console.log(`[Options Debug] filterLanguages: Filtering for "${lowerQuery}". Map size: ${Object.keys(ALL_LANGUAGE_NAMES_MAP).length}`);
    // --- END DEBUG LOG ---
    ...
    // --- DEBUG LOG ---
    if (DEBUG) console.log(`[Options Debug] filterLanguages: Found ${results.length} results.`);
    // --- END DEBUG LOG ---
    return results;
  }
  function showAutocompleteSuggestions(inputElement, suggestions) {
    // --- DEBUG LOG ---
    if (DEBUG) console.log(`[Options Debug] showAutocompleteSuggestions called with ${suggestions.length} suggestions for input:`, inputElement.id);
    // --- END DEBUG LOG ---
    ...
    // --- DEBUG LOG ---
    if (DEBUG) console.log(`[Options Debug] showAutocompleteSuggestions: Dropdown positioned at top: ${autocompleteDropdown.style.top}, left: ${autocompleteDropdown.style.left}, width: ${autocompleteDropdown.style.width}`);
    // --- END DEBUG LOG ---
    activeAutocompleteInput = inputElement;
  }
  function hideAutocompleteSuggestions() { if (autocompleteDropdown) { autocompleteDropdown.style.display = 'none'; highlightedAutocompleteIndex = -1; const currentHighlighted = autocompleteDropdown.querySelector('.autocomplete-item.selected'); if (currentHighlighted) { currentHighlighted.classList.remove('selected'); } } activeAutocompleteInput = null; }
  function selectAutocompleteSuggestion(itemElement, inputElement) {
    ...
    // --- DEBUG LOG ---
    if (DEBUG) console.log(`[Options Debug] selectAutocompleteSuggestion: Selecting "${languageName}" (Code: ${languageCode}) for input #${inputElement.id}`);
    // --- END DEBUG LOG ---
    inputElement.value = languageName;
    const flagImg = inputElement.parentElement?.querySelector('.language-flag');
    if (flagImg && languageCode) { flagImg.src = `${SVG_PATH_PREFIX}${languageCode.toLowerCase()}.svg`; flagImg.alt = `${languageName} flag`; flagImg.onerror = function() { this.src = FALLBACK_SVG_PATH; this.alt = 'Flag not found'; }; }
  ...
  function handleAutocompleteKeydown(event) { if (!autocompleteDropdown || autocompleteDropdown.style.display === 'none') return; const items = autocompleteDropdown.querySelectorAll('.autocomplete-item'); if (items.length === 0) return; if (event.key === 'ArrowDown') { event.preventDefault(); highlightedAutocompleteIndex = (highlightedAutocompleteIndex + 1) % items.length; updateAutocompleteHighlight(items); items[highlightedAutocompleteIndex].scrollIntoView({ block: 'nearest' }); } else if (event.key === 'ArrowUp') { event.preventDefault(); highlightedAutocompleteIndex = (highlightedAutocompleteIndex - 1 + items.length) % items.length; updateAutocompleteHighlight(items); items[highlightedAutocompleteIndex].scrollIntoView({ block: 'nearest' }); } else if (event.key === 'Enter') { if (highlightedAutocompleteIndex > -1 && activeAutocompleteInput) { event.preventDefault(); selectAutocompleteSuggestion(items[highlightedAutocompleteIndex], activeAutocompleteInput); } } else if (event.key === 'Escape') { hideAutocompleteSuggestions(); event.preventDefault(); } }
  ...
  function handleGlobalClick(event) { const clickedInsideInputWrapper = activeAutocompleteInput ? activeAutocompleteInput.closest('.language-input-wrapper').contains(event.target) : false; if (activeAutocompleteInput && (clickedInsideInputWrapper || (autocompleteDropdown && autocompleteDropdown.contains(event.target)))) { return; } hideAutocompleteSuggestions(); }
  function setupAutocomplete(inputElement) {
    inputElement.addEventListener('input', (event) => {
      const query = event.target.value;
      // --- DEBUG LOG ---
      if (DEBUG) console.log(`[Options Debug] Input event on #${inputElement.id}. Query: "${query}". Checking map size: ${Object.keys(ALL_LANGUAGE_NAMES_MAP).length}`);
      // --- END DEBUG LOG ---
      const suggestions = filterLanguages(query);
      // --- DEBUG LOG ---
      if (DEBUG) console.log(`[Options Debug] Input event on #${inputElement.id}. Suggestions found: ${suggestions.length}`);
      // --- END DEBUG LOG ---
      if (query.length > 0 && suggestions.length > 0) { showAutocompleteSuggestions(event.target, suggestions); }
      else { hideAutocompleteSuggestions(); }
      handleLanguageTextChange(event); // Update flag/state
    });
    inputElement.addEventListener('focus', (event) => { const query = event.target.value; if (query.length > 0) { const suggestions = filterLanguages(query); if (suggestions.length > 0) { showAutocompleteSuggestions(event.target, suggestions); } else { hideAutocompleteSuggestions(); } } });
  ...
  function renderLanguageOptions() {
    if (!languageSelectionArea) return; languageSelectionArea.innerHTML = '';
    // --- DEBUG LOG ---
    if (DEBUG) console.log(`[Options Debug] renderLanguageOptions: Rendering ${currentAvailableLanguages.length} languages. SVG Prefix: ${SVG_PATH_PREFIX ? 'Loaded' : 'Not Loaded'}`);
    // --- END DEBUG LOG ---
    currentAvailableLanguages.forEach((langName, index) => {
      ...
      if (languageCode && SVG_PATH_PREFIX) { flagImg.src = `${SVG_PATH_PREFIX}${languageCode.toLowerCase()}.svg`; flagImg.alt = `${langName} flag`; flagImg.onerror = function() { this.src = FALLBACK_SVG_PATH || ''; this.alt = 'Flag not found'; if (DEBUG) console.warn(`[Options Debug] Flag onerror for ${langCode}`); }; }
      else { flagImg.src = FALLBACK_SVG_PATH || ''; flagImg.alt = 'Flag not found'; if (DEBUG && langName && SVG_PATH_PREFIX) console.warn(`[Options Debug] No valid code/path for language "${langName}"`); }
  ...
  function handleLanguageTextChange(event) {
    ...
    if (idx >= 0 && idx < currentAvailableLanguages.length) { currentAvailableLanguages[idx] = newLangName.trim(); } else { console.error("handleLanguageTextChange invalid index:", idx); return; }
  ...
  function removeLanguage(indexToRemove) { if (indexToRemove < 0 || indexToRemove >= currentAvailableLanguages.length) return; currentAvailableLanguages.splice(indexToRemove, 1); renderLanguageOptions(); }

  // --- Drag and Drop Handlers ---
  function handleDragStart(event) { const languageOptionElement = event.target.closest('.language-option'); if (!languageOptionElement) { event.preventDefault(); return; } draggedItemIndex = parseInt(languageOptionElement.dataset.index, 10); event.dataTransfer.setData('text/plain', draggedItemIndex); event.dataTransfer.effectAllowed = 'move'; languageOptionElement.classList.add('dragging'); if (DEBUG) console.log('[Options Debug] Drag start for index:', draggedItemIndex); }
  function handleDragOver(event) { event.preventDefault(); const targetElement = event.target.closest('.language-option'); if (!targetElement || targetElement.classList.contains('dragging')) { if (dragOverElement) { dragOverElement.classList.remove('drag-over-top', 'drag-over-bottom'); dragOverElement = null; } return; } const rect = targetElement.getBoundingClientRect(); const mouseY = event.clientY; const midpoint = rect.top + rect.height / 2; if (dragOverElement && dragOverElement !== targetElement) { dragOverElement.classList.remove('drag-over-top', 'drag-over-bottom'); } if (mouseY < midpoint) { targetElement.classList.add('drag-over-top'); targetElement.classList.remove('drag-over-bottom'); } else { targetElement.classList.add('drag-over-bottom'); targetElement.classList.remove('drag-over-top'); } dragOverElement = targetElement; event.dataTransfer.dropEffect = 'move'; }
  function handleDragLeave(event) { if (event.relatedTarget && event.relatedTarget.closest('.language-option') === event.target.closest('.language-option')) { return; } if (dragOverElement) { dragOverElement.classList.remove('drag-over-top', 'drag-over-bottom'); dragOverElement = null; } }
  function handleDrop(event) { event.preventDefault(); if (dragOverElement) { dragOverElement.classList.remove('drag-over-top', 'drag-over-bottom'); dragOverElement = null; } const droppedItemIndex = parseInt(event.dataTransfer.getData('text/plain'), 10); const targetElement = event.target.closest('.language-option'); if (!targetElement || droppedItemIndex === null || droppedItemIndex === undefined) { if (DEBUG) console.warn('[Options Debug] Drop failed: Invalid target or index.'); return; } const targetIndex = parseInt(targetElement.dataset.index, 10); const rect = targetElement.getBoundingClientRect(); const mouseY = event.clientY; const midpoint = rect.top + rect.height / 2; let newIndex = (mouseY < midpoint) ? targetIndex : targetIndex + 1; newIndex = Math.max(0, Math.min(newIndex, currentAvailableLanguages.length)); if (newIndex === droppedItemIndex || newIndex === droppedItemIndex + 1) { if (DEBUG) console.log('[Options Debug] Dropped onto original position.'); return; } if (DEBUG) console.log(`[Options Debug] Dropped index ${droppedItemIndex} onto target index ${targetIndex}. New index: ${newIndex}.`); const [draggedLanguage] = currentAvailableLanguages.splice(droppedItemIndex, 1); const insertionIndex = newIndex > droppedItemIndex ? newIndex - 1 : newIndex; currentAvailableLanguages.splice(insertionIndex, 0, draggedLanguage); if (DEBUG) console.log('[Options Debug] Array after splice:', [...currentAvailableLanguages]); renderLanguageOptions(); }
  function handleDragEnd(event) { const languageOptionElement = event.target.closest('.language-option'); if (languageOptionElement) { languageOptionElement.classList.remove('dragging'); } if (dragOverElement) { dragOverElement.classList.remove('drag-over-top', 'drag-over-bottom'); dragOverElement = null; } draggedItemIndex = null; if (DEBUG) console.log('[Options Debug] Drag end.'); }
  ...
  async function loadSettings() {
    ...
    } catch(e) { DEBUG = false; } // Default if error
    console.log("[Options Debug] Starting loadSettings. Initial DEBUG state:", DEBUG);
    ...
    try {
      ...
      if (chrome.runtime.lastError || !langDataResponse?.ALL_LANGUAGE_NAMES_MAP) {
        ...
        if (statusMessage) { statusMessage.textContent = `Error loading language list. Autocomplete/flags disabled.`; statusMessage.className = 'status-message error'; }
        ALL_LANGUAGE_NAMES_MAP = {}; SVG_PATH_PREFIX = ''; FALLBACK_SVG_PATH = '';
        langError = true;
      } else {
        ...
        if (DEBUG) console.log(`[Options Debug] Fetched ${Object.keys(ALL_LANGUAGE_NAMES_MAP).length} languages. Map keys sample:`, Object.keys(ALL_LANGUAGE_NAMES_MAP).slice(0, 5));
      }
      ...
      if (chrome.runtime.lastError || !modelsResponse?.models) {
        ...
        if (statusMessage) { statusMessage.textContent = `Error loading models list. Using defaults.`; statusMessage.className = 'status-message error'; }
        currentModels = [...DEFAULT_MODEL_OPTIONS];
      } else {
        currentModels = modelsResponse.models;
        if (DEBUG) console.log(`[Options Debug] Fetched ${currentModels.length} models.`);
      }
      ...
      if (chrome.runtime.lastError || settingsResponse?.error) {
        ...
        if (statusMessage) { statusMessage.textContent = `Error loading settings: ${chrome.runtime.lastError?.message || settingsResponse?.error}. Using defaults.`; statusMessage.className = 'status-message error'; }
        ...
        currentAvailableLanguages = DEFAULT_PREPOPULATE_LANGUAGES.filter(name => findLanguageByName(name)); // Use filter with potentially loaded map
        ...
        if (DEBUG) { const settingsToLog = { ...data }; if (settingsToLog.apiKey) settingsToLog.apiKey = '[API Key Hidden]'; console.log("[Options Debug] Loaded settings from background:", settingsToLog); }
        ...
        if (!langError) { // Only filter if language map loaded
          const validSavedLanguages = currentAvailableLanguages.filter(name => findLanguageByName(name));
          if (validSavedLanguages.length < currentAvailableLanguages.length && DEBUG) { console.warn("[Options Debug] Some saved languages were invalid and removed during load."); }
        ...
        if (!langError && statusMessage) { // Only show success if everything loaded
      ...
      if (statusMessage) { statusMessage.textContent = `Error: ${error.message}`; statusMessage.className = 'status-message error'; }
  ...
  function saveSettings() {
    ...
    const allEnteredNames = Array.from(languageInputs).map(input => input.value.trim()).filter(lang => lang !== '');
    let invalidLanguagesFound = false;
    if (allEnteredNames.length > languagesToSave.length) {
      invalidLanguagesFound = true;
      const invalidNames = allEnteredNames.filter(name => !findLanguageByName(name)).join(', ');
      if (DEBUG) console.warn(`[Options Debug] Invalid languages entered: ${invalidNames}`);
      alert(`Warning: The following entered languages are not recognized and will not be saved: ${invalidNames}\nPlease correct them or remove them.`);
      languageInputs.forEach(input => { input.style.borderColor = (input.value.trim() !== '' && !findLanguageByName(input.value.trim())) ? 'red' : ''; });
    ...
    const settingsToSave = {
      ...
      debug, bulletCount, availableLanguages: languagesToSave, // Save only valid ones
    ...
    if (DEBUG) { const settingsToLog = { ...settingsToSave }; if (settingsToLog.apiKey) settingsToLog.apiKey = '[API Key Hidden]'; console.log("[Options Debug] Saving data:", settingsToLog); }

    chrome.storage.sync.set(settingsToSave, () => {
      if (chrome.runtime.lastError) { if (statusMessage) { statusMessage.textContent = `Error saving: ${chrome.runtime.lastError.message}`; statusMessage.className = 'status-message error'; } console.error("Error saving settings:", chrome.runtime.lastError); }
      else {
        if (statusMessage) { statusMessage.textContent = 'Options saved!'; statusMessage.className = 'status-message success'; }
      ...
      if (statusMessage) { setTimeout(() => { if (statusMessage) { statusMessage.textContent = ''; statusMessage.className = 'status-message'; } }, 2000); }
    });
  }

  /** Resets settings to defaults */
  function resetToDefaults() {
    if (DEBUG) console.log("[Options Debug] Resetting options to defaults (excluding API key)...");
    currentModels = [...DEFAULT_MODEL_OPTIONS]; currentSelectedModel = currentModels.length > 0 ? currentModels[0].id : '';
    currentAvailableLanguages = DEFAULT_PREPOPULATE_LANGUAGES.filter(name => findLanguageByName(name)); // Use filter with potentially loaded map
    if (currentAvailableLanguages.length === 0) { currentAvailableLanguages.push(""); }
    currentCustomFormatInstructions = DEFAULT_FORMAT_INSTRUCTIONS;
    ...
    if (statusMessage) { statusMessage.textContent = 'Defaults Reset & Saved!'; statusMessage.className = 'status-message success'; setTimeout(() => { if (statusMessage) { statusMessage.textContent = ''; statusMessage.className = 'status-message'; } }, 2500); }
  }

  // --- Collapsible Section Logic ---
  function setupCollapsible() {
    if (!advancedOptionsToggle || !advancedOptionsContent) { if (DEBUG) console.warn("[Options Debug] Collapsible elements not found."); return; }
    ...
    const toggleSection = () => {
      ...
      if (!isExpanded) { setTimeout(() => { advancedOptionsToggle.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); }, 100); }
    };
