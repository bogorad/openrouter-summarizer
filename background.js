    ...
    chrome.storage.sync.get('debug', result => {
      if (!chrome.runtime.lastError) {
        ...
        if (newDebugState !== DEBUG) {
          if (newDebugState) console.log('[LLM Background] DEBUG state updated to true via storage check.'); // Log activation
          DEBUG = newDebugState; // Update global DEBUG
        }
      } else {
        console.error("[LLM Background] Error fetching debug state in listener:", chrome.runtime.lastError);
      }
  ...
  getDebugPromise.then(() => {
    ...
    if (request.action === "getSettings") {
      ...
      chrome.storage.sync.get(keysToFetch, (data) => {
        ...
        const settings = {
          apiKey: data.apiKey || '',
          model: data.model || (DEFAULT_MODEL_OPTIONS.length > 0 ? DEFAULT_MODEL_OPTIONS[0].id : ''),
          ...
          : DEFAULT_PREPOPULATE_LANGUAGES.filter(name => ALL_LANGUAGE_NAMES_MAP[name.toLowerCase()]),
    ...
    // --- Get Chat Context ---
      ...
      chrome.storage.sync.get(['models'], syncData => { // Only fetch models needed here
        chrome.storage.session.get(['chatContext'], (sessionData) => {
          ...
          if (DEBUG) {
            ...
            try { if (retrievedData !== undefined) { retrievedDataString = JSON.stringify(retrievedData); if (retrievedDataString.length > 500) { retrievedDataString = retrievedDataString.substring(0, 500) + '... [TRUNCATED]'; } } } catch (e) { retrievedDataString = '[Error stringifying retrievedData]'; }
            console.log('[LLM Background] Raw retrieved sessionData.chatContext:', retrievedDataString);
          }
          const storedContext = retrievedData || {};
          if (DEBUG) console.log('[LLM Background] storedContext after fallback:', storedContext);

          let modelsToSend = [];
          if (Array.isArray(syncData.models) && syncData.models.length > 0 && syncData.models.every(m => typeof m === 'string')) {
            modelsToSend = syncData.models.map(id => { const defaultModel = DEFAULT_MODEL_OPTIONS.find(m => m.id === id); return { id: id, label: defaultModel ? defaultModel.label : id }; });
...
async function handleLLMStream(request, sender) {
  ...
  chrome.storage.sync.get(['apiKey'], async (config) => {
    ...
    if (!apiKey) {
      ...
      } else { console.error('[LLM Background Stream] Cannot send API key error: sender tab ID missing.'); }
      return;
    }
    ...
    if (apiMessages.length === 0) {
      ...
      if (sender?.tab?.id) { chrome.tabs.sendMessage(sender.tab.id, { action: "llmChatStreamError", error: "Internal error: No valid messages to send." }); }
      return;
    }
    const payload = { model: request.model, messages: apiMessages, stream: true };
    ...
    try {
      ...
      while (!streamEnded) {
        let tabExists = false;
        if (sender?.tab?.id) {
          try { await chrome.tabs.get(sender.tab.id); tabExists = true; }
          catch (e) { if (DEBUG) console.log(`[LLM Background Stream] Sender tab ${sender.tab.id} closed. Aborting stream read.`); streamEnded = true; try { reader.cancel(); } catch (cancelError) { if (DEBUG) console.warn('[LLM Background Stream] Failed to cancel reader:', cancelError); } break; }
        } else { if (DEBUG) console.log(`[LLM Background Stream] Sender tab information missing. Aborting stream read.`); streamEnded = true; try { reader.cancel(); } catch (cancelError) { if (DEBUG) console.warn('[LLM Background Stream] Failed to cancel reader:', cancelError); } break; }
        if (!tabExists) continue;

        const { done, value } = await reader.read();
        if (done) { if (DEBUG) console.log('[LLM Background Stream] Stream finished (reader done).'); streamEnded = true; break; }
        ...
        for (let line of lines) {
          ...
          if (data === "[DONE]") { if (DEBUG) console.log('[LLM Background Stream] Received [DONE] marker.'); streamEnded = true; break; }
          try {
            let parsed = JSON.parse(data); let delta = parsed.choices?.[0]?.delta?.content || "";
            if (parsed.model) modelName = parsed.model;
            if (delta && sender?.tab?.id) { chrome.tabs.sendMessage(sender.tab.id, { action: "llmChatStreamChunk", delta: delta }); }
          } catch (e) { if (DEBUG) console.warn('[LLM Background Stream] Skipping non-JSON data line or parse error:', data, 'Error:', e); }
        }
      ...
      if (sender?.tab?.id) {
        try { await chrome.tabs.get(sender.tab.id); chrome.tabs.sendMessage(sender.tab.id, { action: "llmChatStreamDone", model: modelName }); }
        catch (e) { if (DEBUG) console.log(`[LLM Background Stream] Sender tab ${sender.tab.id} closed before sending DONE.`); }
      }

    } catch (e) {
      if (DEBUG) console.error('[LLM Background Stream] Fetch/Stream Error:', e);
      if (sender?.tab?.id) {
        try { await chrome.tabs.get(sender.tab.id); chrome.tabs.sendMessage(sender.tab.id, { action: "llmChatStreamError", error: String(e) }); }
        catch (tabError) { if (DEBUG) console.log(`[LLM Background Stream] Sender tab ${sender.tab.id} closed before sending error.`); }
      }
    }
