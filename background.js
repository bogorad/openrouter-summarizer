...
chrome.storage.sync.get("debug", (data) => { DEBUG = !!data.debug; });

chrome.runtime.onInstalled.addListener(() => {
  chrome.contextMenus.create({ id: "sendToLLM", title: "Send to LLM", contexts: ["all"] });
  chrome.storage.sync.get(["apiKey", "debug"], (data) => {
    DEBUG = !!data.debug;
    if (!data.apiKey) { chrome.runtime.openOptionsPage(); }
  });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  chrome.storage.sync.get("debug", (result) => {
    ...
    if (request.action === "getSettings") {
       ...
       const keysToFetch = [
         "apiKey", "model", "models", "debug", "bulletCount", "language_info",
         PROMPT_STORAGE_KEY_CUSTOM_FORMAT, PROMPT_STORAGE_KEY_PREAMBLE,
         PROMPT_STORAGE_KEY_POSTAMBLE, PROMPT_STORAGE_KEY_DEFAULT_FORMAT,
       ];
       chrome.storage.sync.get(keysToFetch, (data) => {
         ...
         if (Array.isArray(data.models) && data.models.length > 0 && data.models.every(m => typeof m === "object" && m !== null && typeof m.id === "string")) {
         ...
         const settings = {
           apiKey: data.apiKey || "", model: finalSelectedModel, models: loadedModels, debug: DEBUG,
           bulletCount: data.bulletCount || DEFAULT_BULLET_COUNT,
           language_info: Array.isArray(data.language_info) ? data.language_info : [],
         ...
         if (DEBUG) console.log("[LLM Background] Sending settings response - OK.");
    ...
    // --- getChatContext Handler ---
       ...
       chrome.storage.sync.get(["models", "language_info"], (syncData) => { // Fetch language_info here
         ...
         chrome.storage.session.get(["chatContext"], (sessionData) => {
           ...
           if (Array.isArray(syncData.models) && syncData.models.length > 0 && syncData.models.every(m => typeof m === "object" && m !== null && typeof m.id === "string")) {
             modelsToSend = syncData.models.map(m => ({ id: m.id, label: typeof m.label === "string" && m.label.trim() !== "" ? m.label : m.id }));
           }
    ...
    // --- llmChatStream Handler ---
       ...
       chrome.storage.sync.get(["apiKey", "models"], (storageResult) => {
         ...
         if (!apiKey || typeof apiKey !== "string" || apiKey.trim() === "") { sendResponse({ status: "error", message: "API key required." }); return; }
         if (!request.model || typeof request.model !== "string" || request.model.trim() === "" || !modelIds.includes(request.model)) { sendResponse({ status: "error", message: "Valid model required." }); return; }
         ...
         fetch("https://openrouter.ai/api/v1/chat/completions", { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json", "HTTP-Referer": "https://github.com/bogorad/openrouter-summarizer", "X-Title": "OR-Summ" }, body: JSON.stringify(payload), signal: signal })
           ...
           .catch(error => {
             ...
             else { if (DEBUG) console.log("[LLM Background] Chat fetch aborted."); }
             try { sendResponse({ status: "error", message: error.message }); } catch (e) { if (DEBUG) console.warn("Failed to send chat error response:", e.message); }
    ...
    // --- abortChatRequest Handler ---
       ...
       chrome.storage.session.get("abortController", (data) => {
         ...
         if (controller && typeof controller.abort === 'function') {
           ...
           chrome.storage.session.remove("abortController");
           try { sendResponse({ status: "aborted" }); } catch (e) { if (DEBUG) console.warn("Failed to send abort response:", e.message); }
         } else {
           if (DEBUG) console.log("No active request or valid controller to abort.");
           try { sendResponse({ status: "no active request" }); } catch (e) { if (DEBUG) console.warn("Failed to send no active request response:", e.message); }
         }
       });
       return true;
    }
    // --- setChatContext Handler ---
       ...
       chrome.storage.session.set({ chatContext: request }, () => {
         if (DEBUG) console.log("[LLM Background] Chat context set in session storage:", request);
         try { sendResponse({ status: "ok" }); } catch (e) { if (DEBUG) console.warn("Failed to send setChatContext response:", e.message); }
       });
       return true;
    }
    // --- openChatTab Handler ---
    ...
    // --- openOptionsPage Handler ---
       ...
       try { sendResponse({ status: "options page opened" }); } catch (e) { if (DEBUG) console.warn("Failed to send openOptionsPage response:", e.message); }
    }
    // --- requestSummary Handler ---
      ...
      chrome.storage.sync.get(
        [ "apiKey", "model", "bulletCount", "language_info", PROMPT_STORAGE_KEY_CUSTOM_FORMAT, PROMPT_STORAGE_KEY_PREAMBLE, PROMPT_STORAGE_KEY_POSTAMBLE, PROMPT_STORAGE_KEY_DEFAULT_FORMAT, "models" ],
        (data) => {
          // --- ADD DEBUG LOGGING HERE ---
          if (DEBUG) {
            console.log("[LLM Background] Data retrieved for summary request:", {
              ...data,
              apiKey: data.apiKey ? "[API Key Hidden]" : "undefined", // Hide key in log
            });
            console.log("[LLM Background] Type of retrieved apiKey:", typeof data.apiKey);
          }
          // --- END DEBUG LOGGING ---

          const apiKey = data.apiKey;
          const model = data.model;
          const language_info = Array.isArray(data.language_info) ? data.language_info : [];
          const models = data.models || DEFAULT_MODEL_OPTIONS;
          const modelIds = models.map((m) => m.id);

          // Validate apiKey
          if (!apiKey || typeof apiKey !== "string" || apiKey.trim() === "") {
            console.error("[LLM Background] API key is missing or invalid for summary request. Retrieved value:", apiKey); // Log the problematic value
            if (sender.tab?.id) {
                chrome.tabs.sendMessage(sender.tab.id, { action: "summaryResult", requestId: request.requestId, error: "API key is required and must be a non-empty string." }, () => {
                    if (chrome.runtime.lastError && DEBUG && !isTabClosedError(chrome.runtime.lastError)) { console.error(`[LLM Background] Error sending API key error to tab ${sender.tab.id}: ${chrome.runtime.lastError.message}`); }
                });
            }
          ...
          if (!model || typeof model !== "string" || model.trim() === "" || !modelIds.includes(model)) {
            ...
            if (sender.tab?.id) {
             ...
             // Send error back immediately
            sendResponse({ status: "error", message: "Model not selected or invalid." });
            return; // Stop processing
          }
          ...
          fetch("https://openrouter.ai/api/v1/chat/completions", { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json", "HTTP-Referer": "https://github.com/bogorad/openrouter-summarizer", "X-Title": "OR-Summ" }, body: JSON.stringify(payload) })
            ...
            .then((data) => {
              ...
              if (sender.tab?.id) {
                chrome.tabs.sendMessage(sender.tab.id, completeResponse, () => {
                  if (chrome.runtime.lastError) {
                    if (isTabClosedError(chrome.runtime.lastError)) { if (DEBUG) { console.log(`[LLM Background] Tab ${sender.tab.id} closed before summary response could be sent.`, chrome.runtime.lastError.message); } }
                    else { console.error(`[LLM Background] Error sending summary response to tab ${sender.tab.id}:`, chrome.runtime.lastError.message); }
                  }
                });
            ...
            .catch((error) => {
              ...
              const errorResponse = { action: "summaryResult", requestId: request.requestId, error: error.message, language_info: language_info, fullResponse: DEBUG ? { error: error.message } : "[Debug data omitted for brevity]" };
              ...
              if (sender.tab?.id) {
                chrome.tabs.sendMessage(sender.tab.id, errorResponse, () => {
                  if (chrome.runtime.lastError) {
                    if (isTabClosedError(chrome.runtime.lastError)) { if (DEBUG) { console.log(`[LLM Background] Tab ${sender.tab.id} closed before summary error response could be sent.`, chrome.runtime.lastError.message); } }
                    else { console.error(`[LLM Background] Error sending summary error response to tab ${sender.tab.id}:`, chrome.runtime.lastError.message); }
                  }
                });
              } else { if (DEBUG) console.warn("[LLM Background] No tab ID available to send summary error response."); }
            });
    ...
    // --- healthCheck Handler ---
       ...
       chrome.storage.sync.get(["apiKey", "model", "models"], (data) => {
         ...
         const models = data.models || DEFAULT_MODEL_OPTIONS;
         const modelIds = models.map((m) => m.id);
         if (!apiKey || typeof apiKey !== "string" || apiKey.trim() === "") { sendResponse({ status: "error", message: "API key not configured." }); return; }
         if (!model || typeof model !== "string" || model.trim() === "" || !modelIds.includes(model)) { sendResponse({ status: "error", message: "Default model invalid." }); return; }
         if (DEBUG) console.log("[LLM Background] Sending healthCheck response - OK.");
...
function getSystemPrompt(
  ...
  const {
      ...
      DEFAULT_POSTAMBLE_TEXT,
      DEFAULT_FORMAT_INSTRUCTIONS
  } = typeof constants !== 'undefined' ? constants : { /* Fallbacks */ };
